...
const getCompanyAge = () => {
  return time.since(new Date("2023-09-11T00:00:00.000Z"), {
    locale: "fr",
    format: "day",
    maxPartCount: 3,
  })
...
export default function Contact({
  ...
  const [frenchHour, setFrenchHour] = React.useState(getFrenchHour())
  const [companyAge, setCompanyAge] = React.useState(getCompanyAge())
  ...
  // update the French hour
  React.useEffect(() => {
    const setFrenchHourInterval = setInterval(() => {
      setFrenchHour(getFrenchHour())
    }, 1000 * 5)

    const setCompanyAgeInterval = setInterval(
      () => {
        setCompanyAge(getCompanyAge())
      },
      1000 * 60 * 60 * 24
    )

    return () => {
      clearInterval(setFrenchHourInterval)
      clearInterval(setCompanyAgeInterval)
    }
  }, [])
  ...
  return (
    <>
      ...
      {/*/>*/}
      <ScrollArea className="w-screen h-full" ref={scrollAreaRef}>
        <div className="container mx-auto py-3 animate-show opacity-0">
          <div className="flex flex-col xl:flex-row gap-3">
            <div className="basis-full">
              <h2 className="text-3xl my-3">Me contacter</h2>
              <Alert className="mb-2 shad0w">
                <AlertTriangle className="h-4 w-4" />
                <AlertTitle>
                  Je ne réponds pas aux appels téléphoniques
                </AlertTitle>
                <AlertDescription>
                  S'il vous plait contactez-moi textuellement pour un meilleur
                  taux de réponse de ma part.
                </AlertDescription>
              </Alert>
              <ul>
                <li>
            ...
            <div className="basis-full">
              ...
              <address>
                <Card className="w-full shad0w list p-4">
                  Camille Abella <br />
                  5 Rue Agusanum 66240 Saint-Estève <br />
                  Europe/France <br />
                </Card>
              </address>
              <span className="text-2xl">
                Heure française:&nbsp;
                <span className="font-mono">{frenchHour}</span>
              </span>
            </div>
          </div>
          <div>
            <h2 className="text-3xl my-3">Mon entreprise</h2>
            <List>
              <li>
                <strong>Nom</strong>: CAMILLE ABELLA
              </li>
              <li>
                <strong>Type</strong>: Micro-entreprise / Freelance
              </li>
              <li>
                <strong>SIRET</strong>: <code>979 392 164 00013</code>
              </li>
              <li>
                <strong>Activité</strong>: Prestation de développement web / jeu
                vidéo, Fullstack JS
              </li>
              <li>
                <strong>Date de création</strong>: <code>11/09/2023</code> (il y
                a&nbsp;
                {companyAge})
              </li>
            </List>
